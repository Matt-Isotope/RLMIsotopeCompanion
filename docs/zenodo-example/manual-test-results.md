...
These tests were performed using the same local LM Studio model with the following system prompts:

- `Geochemistry & Isotopes`;
- `General R Expert`.

The purpose was to check whether the model identifies visible R errors without inventing missing context.
...
## Overall assessment of the manual tests
...
They also show that the model may accept invalid code, make unsupported assumptions about data not included in the selection, incorrectly describe base R behavior, or propose corrections that change the intended behavior. In particular, the result of the `missing_column` test must be interpreted conditionally: the selected code does not prove whether that column exists.

The addin should therefore be used as an interactive code-review aid, not as an automatic R validator or scientific validation system. Every response should be checked against the complete data context and independently verified in R.
